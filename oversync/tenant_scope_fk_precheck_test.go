...
func TestFKPrecheck_TenantScopeColumn_ScopesParentExistence(t *testing.T) {
	if testing.Short() {
	...
	schemaName := "tenant_fk_" + suffix
	userB := "user-b-" + suffix
	userAScoped := "user-a-scoped-" + suffix
	userAUnscoped := "user-a-unscoped-" + suffix

	require.NoError(t, createTenantFKPrecheckSchema(ctx, pool, schemaName))
	...
	t.Run("TenantScopingEnabledRejectsCrossUserParent", func(t *testing.T) {
		svc, err := NewSyncService(pool, &ServiceConfig{
			MaxSupportedSchemaVersion: 1,
			AppName:                   "tenant-scope-fk-precheck-test",
	...
	t.Run("TenantScopingDisabledIncorrectlyAcceptsCrossUserParent", func(t *testing.T) {
		svc, err := NewSyncService(pool, &ServiceConfig{
			MaxSupportedSchemaVersion: 1,
			AppName:                   "tenant-scope-fk-precheck-test",
			RegisteredTables: []RegisteredTable{
